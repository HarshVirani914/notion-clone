...
export const fetchNotes = createAsyncThunk(
  ...
  async (user: string) => {
    ...
    const response = await axios.get(
      `http://localhost:3001/document/fetchDocuments/${user}`
    );
...
export const fetchNoteById = createAsyncThunk(
  ...
  async (documentId: number) => {
    try {
      const response = await axios.get(
        `http://localhost:3001/document/fetchDocument/${documentId}`
      );
...
export const fetchDeletedDocuments = createAsyncThunk(
  ...
  async (userId: string) => {
    try {
      const response = await axios.get(
        `http://localhost:3001/document/fetchDeletedDocuments/${userId}`
      );
      return response.data;
    } catch (error) {
...
export const selectAllDeletedDocuments = (state: RootState) =>
  state.notes.deletedDocuments;
